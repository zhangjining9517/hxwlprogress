package com.zjn.dialoglibrary.view;
...
import com.zjn.dialoglibrary.R;
import com.zjn.dialoglibrary.utils.DialogUtils;
/**
 * 消息状态提醒对话框
 */
public class MsgStatusDialog {
    ...
    public MsgStatusDialog(Context context) {
        this(context, new Builder(context));
    }

    public MsgStatusDialog(Context context, Builder builder) {
        mContext = context;
        mBuilder = builder;
        //初始化
        initDialog();
    ...
    private void initDialog() {
        ...
        View mProgressDialogView = inflater.inflate(R.layout.dialog_status_layout, null);// 得到加载view
        mDialog = new Dialog(mContext, R.style.MsgStatusDialog);// 创建自定义样式dialog
        ...
        imageStatus = (ImageView) mProgressDialogView.findViewById(R.id.imageStatus_horizontal);
        tvShow = (TextView) mProgressDialogView.findViewById(R.id.tvShow_horizontal);
        //默认配置
        configView();
    ...
    private void configView() {
        ...
        myGrad.setStroke(DialogUtils.dp2px(mContext, mBuilder.strokeWidth), mBuilder.strokeColor);
        myGrad.setCornerRadius(DialogUtils.dp2px(mContext, mBuilder.cornerRadius));
    ...
    public static final class Builder {
        ...
        public Builder(Context context) {
            mContext = context;
            //默认配置
            backgroundWindowColor = mContext.getResources().getColor(R.color.dialog_transparent);
            backgroundViewColor = mContext.getResources().getColor(R.color.dialog_color_bg);
            strokeColor = mContext.getResources().getColor(R.color.dialog_transparent);
            textColor = mContext.getResources().getColor(R.color.dialog_color_text);
            cornerRadius = 6;
            strokeWidth = 0;
        }

        public MsgStatusDialog build() {
            return new MsgStatusDialog(mContext, this);
        }
